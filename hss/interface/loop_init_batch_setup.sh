...
for p in ${patients[*]}; do
    for d in ${drugs[*]}; do
        conda run -n nephro python batch_setup.py -i Run_Info_"$runname"_"$p"_"$d".json
        # for r in ${radlevels[*]}; do
        #     conda run -n nephro python batch_setup.py -i Run_Info_"$runname"_"$p"_"$d"_Rad_"$r"Gy.json
        # done
    done
done
