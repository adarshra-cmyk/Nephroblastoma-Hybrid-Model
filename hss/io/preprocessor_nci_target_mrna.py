...
if __name__ == '__main__':
    ...
    parser.add_option("-p", "--patient-id", dest = "patient_id", help = "The id of the patient for which miRNA or mRNA data is available")
    parser.add_option("-m", "--mrna", dest = "mrna", help = "Whether to use an mRNA profile directly as opposed to mRNA targets of differential miRNAs")
    ...
    if options.patient_id != "Control":
        if options.mrna:
            Run_Info["mrna"]["mrna_file"] = "mrna_data/%s.gene.quantification.txt" % options.patient_id
            mrna_file = "mrna_hits%s.json" % suffix

            mrnas = pd.read_csv(Run_Info["mrna"]["mrna_file"], sep='\t', header=0)
        else:
